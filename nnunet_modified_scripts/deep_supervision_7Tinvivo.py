...
class MultipleOutputLoss2_SOR_DSC(nn.Module):
    ...
    def forward(self, x, y, epoch):
        ...
        if epoch > self.sor_start_epoch:
            ...
            #Already a segmentation - converted in augmentaion function - this is the Laplacian seg
            t_gt = y[0]

            #Mask the target incase of interpolation errors
            #Convert WM/CSF regions to label 0 in ground truth Laplacian seg. Loss computation ignores regions where ground truth is 0 from dice comp
            t_gt[t_gt == 9] = 0 #Set CSF to zero. Only want Laplacian within GM
        ...
        # Skip the first element. First element is the ground truth laplacian map/seg
        y = y[1:]
